...
public class API {
     ...
     */
    public static int startServer(String serverType, String version, ServerThreadCallback callback) {
        // Create a new server instance using the ServerBuilder
        Server server = new ServerBuilder((serverType + "(" + version + ")")).serverType(serverType).version(version).build();
